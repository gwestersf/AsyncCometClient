...
/**
 * @version $Revision: 1304 $ $Date: 2010-06-21 11:57:17 +0200 (Mon, 21 Jun
 *          2010) $
 */
public class StatisticsHelper implements Runnable {
	...
	public StatisticsHelper() {
		this.operatingSystem = (com.sun.management.OperatingSystemMXBean) ManagementFactory
		...
		List<GarbageCollectorMXBean> garbageCollectors = ManagementFactory
	...
	public boolean startStatistics() {
		// Support for multiple nodes requires to ignore start requests after
		// the first
		...
		synchronized (this) {
			...
			System.err.println("Operative System: " + operatingSystem.getName()
					+ " " + operatingSystem.getVersion() + " "
					+ operatingSystem.getArch());
			System.err.println("JVM : " + System.getProperty("java.vm.vendor")
					+ " " + System.getProperty("java.vm.name") + " runtime "
			...
			System.err.println("System Memory: "
					...
					+ "% used of " + gibiBytes(totalMemory) + " GiB");

			MemoryUsage heapMemoryUsage = heapMemory.getHeapMemoryUsage();
			System.err.println("Used Heap Size: "
					+ mebiBytes(heapMemoryUsage.getUsed()) + " MiB");
			System.err.println("Max Heap Size: "
					+ mebiBytes(heapMemoryUsage.getMax()) + " MiB");
			long youngGenerationHeap = heapMemoryUsage.getMax()
					- oldMemoryPool.getUsage().getMax();
			System.err.println("Young Generation Heap Size: "
					+ mebiBytes(youngGenerationHeap) + " MiB");
			System.err.println("- - - - - - - - - - - - - - - - - - - - ");

			scheduler = Executors.newSingleThreadScheduledExecutor();
			polling = false;
			memoryPoller = scheduler.scheduleWithFixedDelay(this, 0, 500,
	...
	public boolean stopStatistics() {
		synchronized (this) {
			...
			long elapsedJITCTime = jitCompiler.getTotalCompilationTime()
					- startJITCTime;
			long elapsedProcessCPUTime = operatingSystem.getProcessCpuTime()
					- startProcessCPUTime;
			long elapsedTime = System.nanoTime() - startTime;

			long elapsedOldCollectionsTime = oldCollector.getCollectionTime()
					- startOldCollectionsTime;
			long oldCollections = oldCollector.getCollectionCount()
					- startOldCollections;
			long elapsedYoungCollectionsTime = youngCollector
					.getCollectionTime() - startYoungCollectionsTime;
			long youngCollections = youngCollector.getCollectionCount()
					- startYoungCollections;
			...
			System.err.println("Elapsed time: "
					+ TimeUnit.NANOSECONDS.toMillis(elapsedTime) + " ms");
			System.err.println("\tTime in JIT compilation: " + elapsedJITCTime
					+ " ms");
			System.err.println("\tTime in Young Generation GC: "
					+ elapsedYoungCollectionsTime + " ms (" + youngCollections
					+ " collections)");
			System.err.println("\tTime in Old Generation GC: "
					+ elapsedOldCollectionsTime + " ms (" + oldCollections
					+ " collections)");

			System.err.println("Garbage Generated in Young Generation: "
					+ mebiBytes(totalYoungUsed) + " MiB");
			System.err.println("Garbage Generated in Survivor Generation: "
					+ mebiBytes(totalSurvivorUsed) + " MiB");
			System.err.println("Garbage Generated in Old Generation: "
					+ mebiBytes(totalOldUsed) + " MiB");

			System.err.println("Average CPU Load: "
