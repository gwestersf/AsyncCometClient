 ...
 * 
 * A rather silly java client for Bayeux that prints out messages received on a
 * channel. As the client interactively prompts the user for input to send on
 * the channel, the effect is to echo back the messages.
 */
public class SimpleEchoBayeuxClient {
	...
	public SimpleEchoBayeuxClient(String host, int port, String uri, String who)
		...
		_client = new BayeuxClient("http://" + host + ":" + port + uri,
				LongPollingTransport.create(null, _httpClient));

		_client.getChannel(Channel.META_CONNECT).addListener(
		...
		_client.getChannel(Channel.META_HANDSHAKE).addListener(
				new ClientSessionChannel.MessageListener() {
					@Override
					public void onMessage(ClientSessionChannel channel,
							Message message) {
						if (message.isSuccessful()) {
							_client.getChannel("/foo/alpha").subscribe(
									_alphaListener);
							Object msg = new JSON.Literal("{\"user\":\"" + _who
									+ "\",\"chat\":\"Has joined\"}");
							_client.getChannel("/foo/alpha").publish(msg,
									String.valueOf(_id++));
							_connected = true;
						}
					}
	...
	public void publish(String say) {
		Object msg = new JSON.Literal("{\"user\":\"" + _who + "\",\"chat\":\""
				+ say + "\"}");
	...
	public static void main(String[] args) {
		try {
			// arg0: server url
			String serverCometdUrl = (args.length > 0 ? args[0]
					: "/cometd/cometd");

			// arg1: server port
			int serverPort = (args.length >= 2 ? Integer.valueOf(args[1])
					: 8080);

			// arg2: username (not necessary)
			String user = (args.length >= 3 ? args[2] : "anonymous");

			SimpleEchoBayeuxClient sbc = new SimpleEchoBayeuxClient(
					"localhost", serverPort, serverCometdUrl, user);
			sbc.start();
