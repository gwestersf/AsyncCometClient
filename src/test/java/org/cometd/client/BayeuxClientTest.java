...
public class BayeuxClientTest extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		_bayeux = (BayeuxServerImpl) context.getServletContext().getAttribute(
	...
	public void testClient() throws Exception {
		...
		BayeuxClient client = new BayeuxClient("http://localhost:" + _port
				+ "/cometd", LongPollingTransport.create(null, _httpClient));
		...
		client.getChannel("/meta/*").addListener(
				new ClientSessionChannel.MessageListener() {
					@Override
					public void onMessage(ClientSessionChannel channel,
							Message message) {
	...
	public void testAsync() throws Exception {
		...
		client.getChannel("/foo/bar").subscribe(
				new ClientSessionChannel.MessageListener() {
					@Override
					public void onMessage(ClientSessionChannel channel,
							Message message) {
	...
	public void testRetry() throws Exception {
		final BlockingArrayQueue<Message> queue = new BlockingArrayQueue<Message>(
				100, 100);
		...
		client.getChannel("/**").addListener(
				new ClientSessionChannel.MessageListener() {
					@Override
					public void onMessage(ClientSessionChannel session,
							Message message) {
						if (message.getData() != null
								|| Channel.META_SUBSCRIBE.equals(message
										.getChannel())
								|| Channel.META_DISCONNECT.equals(message
										.getChannel())) {
							queue.offer(message);
						}
					}
		...
		client.setOption(BayeuxClient.BACKOFF_INCREMENT_OPTION,
				backoffIncrement);
		client.handshake();
	...
	public void testPerf() throws Exception {
		...
		for (int i = 0; i < clients.length; i++) {
			final AtomicBoolean connected = new AtomicBoolean();
			final BayeuxClient client = new BayeuxClient("http://localhost:"
					+ _port + "/cometd", LongPollingTransport.create(null,
					_httpClient));
		...
		Thread.sleep(100);
		while (connections.get() < clients.length
				&& (System.currentTimeMillis() - start) < 10000) {
			Thread.sleep(1000);
			System.err.println("connected " + connections.get() + "/"
					+ clients.length);
		}
		...
		System.err.println((received.get() * 1000)
				/ (System.currentTimeMillis() - start0) + " m/s");
	...
	public void testPublish() throws Exception {
		...
		_bayeux.getChannel(channelName).addListener(
				new ServerChannel.MessageListener() {
					@Override
					public boolean onMessage(ServerSession from,
							ServerChannel channel, Mutable message) {
