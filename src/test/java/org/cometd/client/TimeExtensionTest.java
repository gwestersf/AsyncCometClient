...
public class TimeExtensionTest extends TestCase {
	...
	void startServer(Server server) throws Exception {
		...
		context.setBaseResource(Resource.newResource(file.toURI().toURL()));

		ServletHolder comet = context.addServlet(CometdServlet.class,
				"/cometd/*");
		...
		_bayeux = (BayeuxServer) context.getServletContext().getAttribute(
	...
	public void testTimeStamp() throws Exception {
		...
		final BayeuxClient client = new BayeuxClient("http://localhost:" + port
				+ "/cometd", LongPollingTransport.create(null));
	...
	public void testTimeSync() throws Exception {
		...
		// TODO: not all messages get the timesync extension if the server
		// thinks it's accurate enough
		// for (Message message : messages)
		// {
