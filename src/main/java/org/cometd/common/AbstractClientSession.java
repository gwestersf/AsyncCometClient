...
import org.cometd.bayeux.client.ClientSessionChannelListener;
import org.cometd.bayeux.client.MessageListener;
 ...
 * </p>
 * <p>
 * It handles extensions and batching, and provides utility methods to be used
 * by subclasses.
 * </p>
 */
public abstract class AbstractClientSession implements ClientSession {
	 ...
	 * </p>
	 * <p>
	 * Processing the message involves calling the receive {@link Extension
	 * extensions} and the channel
	 * {@link ClientSessionChannel.ClientSessionChannelListener listeners}.
	 * </p>
	 * 
	 * @param message
	 *            the message received.
	 * @param mutable
	 *            the mutable version of the message received
	 */
	public void receive(final Message message, final Message.Mutable mutable) {
		String id = message.getChannel();
		if (id == null)
			throw new IllegalArgumentException(
					"Bayeux messages must have a channel, " + message);
	 ...
	 * </p>
	 */
	protected abstract static class AbstractSessionChannel implements
			...
			for (ClientSessionChannelListener listener : _listeners) {
				if (listener instanceof MessageListener) {
					try {
						...
						logger.error("Notify went wrong", x);
					}
				}
			}
			for (ClientSessionChannelListener listener : _subscriptions) {
				if (listener instanceof MessageListener) {
					if (message.getData() != null) {
						try {
							((MessageListener) listener).onMessage(this,
									message);
						} catch (Exception x) {
							logger.error(
									"Client session listener went wrong on subscriptions",
									x);
						}
					}
				}
