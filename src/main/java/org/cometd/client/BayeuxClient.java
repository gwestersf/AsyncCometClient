...
public class BayeuxClient extends AbstractClientSession implements Bayeux, TransportListener {
	...
	public void handshake(Map<String, Object> handshakeFields) {
		...
		// Pick the first transport for the handshake, it will renegotiate if
		// not right
		ClientTransport initialTransport = transportRegistry
				.getTransport(allowedTransport.get(0));
		updateTransport(initialTransport);
		updateState(State.HANDSHAKING);
		logger.debug("Handshaking with extra fields {}, transport {}",
				handshakeFields, initialTransport);
		handshakeBatch = true;
		send(message);
	}
	...
	public State handshake(Map<String, Object> template, long waitMs) {
		handshake(template);
		waitFor(waitMs, State.CONNECTED, State.CONNECTING, State.DISCONNECTED,
				State.UNCONNECTED);
		return getState();
	}
	...
	protected ChannelId newChannelId(String channelId) {
		...
		return channel == null ? new ChannelId(channelId) : channel
				.getChannelId();
	}
	...
	protected void sendBatch() {
		...
		Queue<Message.Mutable> queue = new LinkedList<Message.Mutable>(
				messageQueue);
		// Do not call messageQueue.clear(), as it can contain new messages
		// added concurrently
	...
	protected void followAdvice() {
		...
		if (advice != null) {
			...
			if (advice.containsKey(Message.INTERVAL_FIELD))
				interval = ((Number) advice.get(Message.INTERVAL_FIELD))
						.longValue();
		}
	...
	protected void processHandshake(Message handshake) {
		...
		if (handshake.isSuccessful()) {
			resetBackoff();
			Object[] serverTransports = (Object[]) handshake
					.get(Message.SUPPORTED_CONNECTION_TYPES_FIELD);
			List<ClientTransport> negotiatedTransports = transportRegistry
					.negotiate(serverTransports, BayeuxClient.BAYEUX_VERSION);
			ClientTransport newTransport = negotiatedTransports.isEmpty() ? null
					: negotiatedTransports.get(0);
	...
	protected boolean scheduleAction(Runnable action, long interval) {
		...
		if (scheduler != null) {
			...
			try {
				scheduler.schedule(action, interval + backoff,
	...
	protected void send(Message.Mutable... messages) {
		...
		for (Iterator<Message.Mutable> iterator = messageList.iterator(); iterator
				.hasNext();) {
